        ...
        <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit"
          ...
          <div className="p-4">
            <div className="flex items-center gap-4 pb-4 border-b">
              ...
              <div className={`bg-gradient-to-br from-purple-500 to-pink-500 p-3 rounded-xl shrink-0 ${invite.company?.logo_url ? 'hidden' : 'flex'} items-center justify-center w-14 h-14`}>
                  ...
                  return (
                    <motion.div key={`card-${invite.invite_id}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                      ...
                      <div className="flex items-start gap-4">
                        {invite.company?.logo_url ? (
                          <img src={invite.company.logo_url.startsWith('http') ? invite.company.logo_url : `https://api-attendance.onesaas.in${invite.company.logo_url}`} alt="logo" className="w-12 h-12 rounded-xl object-cover border border-purple-200 bg-white shrink-0" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                        ) : null}
                        <div className={`bg-gradient-to-br from-purple-500 to-pink-500 p-3 rounded-xl w-12 h-12 items-center justify-center shrink-0 ${invite.company?.logo_url ? 'hidden' : 'flex'}`}><FaBuilding className="text-white text-2xl" /></div>
